...
```
Uses r log transformation on data as a data normalization method. This r log tranformation will be important for making heatmaps. Last line sanity checks length to ensure we are not losing any data.
```{r}
...
This heatmap will display the differences in gene expression for each treatment based on their site of origin. Here, we are looking for similarities in gene expression from
```{r}
rldpvals <- read.csv(file="resheat_RLDandPVALS.csv", row.names=1)
head(rldpvals)
...
Our next heatmap will display, among the genes in commmon for all treatments, upregulation and downregulation based on treament.
```{r}
rldpvals <- read.csv(file="resheat_RLDandPVALS.csv", row.names=1)
head(rldpvals)
