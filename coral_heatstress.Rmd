...
We plot a sample distance heatmap to see how general expression compares from sample to sample. The purpose of this map is to look at how overall expression from each sample is similar to each other.
```{r}
...
#head(pca_s)
```
Here, we are essentially looking at the distance between the two points, to visualize overall variance.
```{r}
...
head(explc)
```
Then assign colors and create a heatmap
```{r}
...
## Conclusion 
Conclusion here (Grace will do this!)
